...
export enum urlFields {
  propritorPhotoUrl = " Propritor photo url",
  shopOfficePhotoUrl = "Shop/Office photo url",
  adhaarPhotoUrl = "Adhar card Photo url",
  addressProofPhotoUrl = "AddressProof Photo url",
}

@Schema({ timestamps: true })
export class GumastaApplication {
  ...
  @Prop({ enum: status, default: status.BLANK, trim: true })
  @IsEnum(status)
  status: status;
...
export type GumastaApplicationDocument = GumastaApplication & Document;
export const GumastaApplicationSchema =
  SchemaFactory.createForClass(GumastaApplication);
