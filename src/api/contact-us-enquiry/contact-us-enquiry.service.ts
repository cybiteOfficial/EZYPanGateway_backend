...
@Injectable()
export class ContactUsEnquiryService {
  [x: string]: any;
  constructor(
    @InjectModel(ContactUsEnquiry.name)
    private ContactUsEnquiryModel: Model<ContactUsEnquiryDocument>,
  ...
  async add(res, req) {
    try {
      ...
      const result = await new this.ContactUsEnquiryModel({
        ...req.body,
      }).save();
      ...
      this.addLogFunction.logAdd(
        ...
        req.userData?.Id || '',
        'CONTACT_US_ENQUIRY',
        'CONTACT_US_ENQUIRY_ADD',
        result._id,
        true,
        200,
        `${
          req.userData?.type || ''
        } added contact-us enquiry at ${currentDate}.`,
        'Contact-us enquiry added successfully.',
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        }  tried to add contact-us enquiries with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
  ...
  async allWithFilters(req, res) {
    try {
      ...
      //-----------------------------------
      let totalData = 0;
      const dataFound = await this.ContactUsEnquiryModel.aggregate(countQuery);
      ...
      const result = await this.ContactUsEnquiryModel.aggregate(query);
      if (result.length) {
        this.addLogFunction.logAdd(
          req,
          req.userData?.type || '',
          req.userData.Id,
          'CONTACT_US_ENQUIRY',
          'CONTACT_US_ENQUIRY_FILTER_PAGINATION',
          req.userData?.Id || '',
          true,
          200,
          `${req.userData?.type || ''} ${
            req.userData?.Id || ''
          } filter data for contact-us enquiry  at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } filter data for view contact-us enquiry with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
