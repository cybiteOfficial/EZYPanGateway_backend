...
import { isvalidUrl } from '../../helper/basicValidation';
import {
  StaticPage,
  StaticPageDocument,
...
export class OtherServiceService {
  ...
  async add(res, req) {
    try {
      const { serviceName, serviceDescription, staticPageId } = req.body;

      const reqParams = ['serviceName', 'serviceDescription', 'staticPageId'];
      ...
      const serviceNameExists = await this.OtherServiceModel.findOne({
        serviceName: serviceName,
        isDeleted: false,
      });

      if (serviceNameExists) {
        throw new HttpException('Service name already exists.', HttpStatus.OK);
      }

      const checkStaticPageExist = await this.StaticPageModel.findOne({
        _id: new mongoose.Types.ObjectId(staticPageId),
        isDeleted: false,
        isActive: true,
      });

      if (!checkStaticPageExist) {
        throw new HttpException('Static page does not exists.', HttpStatus.OK);
      }

      const result = await new this.OtherServiceModel({ ...req.body }).save();
      ...
      this.addLogFunction.logAdd(
        ...
        } added other service ${result.serviceName} at ${currentDate}.`,
        'Other Service added successfully.',
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to add other service with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
  ...
  async list(res, req) {
    try {
      ...
      const OtherServiceFound = await this.OtherServiceModel.aggregate([
        {
          $match: { ...query },
        },
        {
          $addFields: {
            staticPageId: { $toObjectId: '$staticPageId' },
          },
        },
        {
          $lookup: {
            from: 'staticpages',
            localField: 'staticPageId',
            foreignField: '_id',
            as: 'staticPageData',
            pipeline: [
              {
        ...
        },
        {
          $addFields: {
            staticPageName: {
              $cond: {
                if: { $eq: ['$staticPageData', []] },
                then: '',
                else: { $arrayElemAt: ['$staticPageData.name', 0] },
              },
            },
            staticPageUrl: {
              $cond: {
                if: { $eq: ['$staticPageData', []] },
                then: '',
                else: { $arrayElemAt: ['$staticPageData.url', 0] },
              },
            },
            staticPageContent: {
              $cond: {
                if: { $eq: ['$staticPageData', []] },
                then: '',
                else: { $arrayElemAt: ['$staticPageData.pageContent', 0] },
              },
            },
          },
        },
        { $unset: 'staticPageData' },
      ]);

      if (!OtherServiceFound.length) {
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } checked list of other service at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to check list of other service with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async view(id: string, res, req) {
    try {
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } viewed other service ${id} at ${currentDate}.`,
        'Data found successfully.',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to view other service ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async update_by_id(id: string, res, req) {
    try {
      ...
      const OtherServiceFound = await this.OtherServiceModel.findOne({
        _id: new mongoose.Types.ObjectId(id),
        isDeleted: false,
      });
      ...
      const result = await this.OtherServiceModel.findByIdAndUpdate(
        { _id: OtherServiceFound._id },
        {
          $set: { ...req.body },
        },
        { new: true },
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        'OTHER_SERVICE',
        'OTHER_SERVICE_UPDATE',
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } updated other service ${id} at ${currentDate}.`,
  ...
  async delete_by_id(id: string, res, req) {
    try {
      ...
      const dataUpdated = await this.OtherServiceModel.findByIdAndUpdate(
        { _id: new mongoose.Types.ObjectId(id) },
        { $set: { isDeleted: true } },
        { new: true },
      );
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        'OTHER_SERVICE',
        'OTHER_SERVICE_DELETE',
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } deleted other service ${id} at ${currentDate}.`,
        'OtherService deleted successfully.',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to delete other service ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async changeActiveStatus(id: string, res, req) {
    try {
      ...
      const activeStatus = OtherServiceFound.isActive === true ? false : true;
      const statusValue = activeStatus === true ? 'ACTIVE' : 'DEACTIVE';

      const statusChanged = await this.OtherServiceModel.findByIdAndUpdate(
      ...
      if (statusChanged.isActive === activeStatus) {
        this.addLogFunction.logAdd(
          ...
          requestedId,
          'OTHER_SERVICE',
          'OTHER_SERVICE_CHANGE_STATUS',
          id,
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ''
          } status changed of other service ${id} at ${currentDate}.`,
          `Status changed of other service ${id} to ${statusValue} successfully.`,
          req.socket.remoteAddress,
        );
        return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to change status of other service ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async allWithFilters(req, res) {
    try {
      ...
      //-----------------------------------
      let totalData = 0;
      const dataFound = await this.OtherServiceModel.aggregate(countQuery);
      ...
      const result = await this.OtherServiceModel.aggregate(query);
      if (result.length) {
        this.addLogFunction.logAdd(
          req,
          requestedType,
          requestedId,
          'OTHER_SERVICE',
          'OTHER_SERVICE_FILTER_PAGINATION',
          '',
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ''
          }filter data to view other service at ${currentDate}.`,
          'Data Found successfully.',
          req.socket.remoteAddress,
        );
        return res.status(200).send({
