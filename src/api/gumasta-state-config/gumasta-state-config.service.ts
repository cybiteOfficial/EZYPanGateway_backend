...
@Injectable()
export class GumastaStateConfigService {
  [x: string]: any;
  constructor(
    @InjectModel(GumastaStateConfig.name)
    private GumastaStateConfigModel: Model<GumastaStateConfigDocument>,
  ...
  async add(res, req) {
    try {
      ...
      const stateExists = await this.GumastaStateConfigModel.findOne({
        state: req.body.state.toUpperCase(),
        isDeleted: false,
      });
      if (stateExists) {
        throw new HttpException("state already exist.", HttpStatus.OK);
      }

      const result = await new this.GumastaStateConfigModel({
        ...req.body,
      }).save();

      if (!result) {
        throw new HttpException("Unable to add state.", HttpStatus.OK);
      }
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ""} ${
          req.userData?.contactNumber || ""
        } tried to add state with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
  ...
  async list(res, req) {
    try {
      ...
      const dataFound = await this.GumastaStateConfigModel.find({
        ...query,
      });
      if (!dataFound.length) {
        throw new HttpException("States not found.", HttpStatus.OK);
      }
      this.addLogFunction.logAdd(
        req,
        requestedType,
        requestedId,
        "GUMASTA_STATE_CONFIG",
        "GUMASTA_STATE_CONFIG_LIST",
        "",
        true,
        200,
        `${requestedType} ${
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ""} ${
          req.userData?.contactNumber || ""
        } tried to check list of gumasta state with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
        errData.resData.message,
        req.socket.remoteAddress
  ...
  async view(id, res, req) {
    try {
      ...
      const dataFound = await this.GumastaStateConfigModel.findOne({
        ...query,
      });
      if (!dataFound) {
        throw new HttpException("States not found.", HttpStatus.OK);
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        "GUMASTA_STATE_CONFIG",
        "GUMASTA_STATE_CONFIG_VIEW_ADMIN",
        "",
        true,
        200,
        `${requestedType} ${
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ""} ${
          req.userData?.contactNumber || ""
        } tried to view  gumasta state with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
        errData.resData.message,
        req.socket.remoteAddress
  ...
  async update_by_id(id: string, res, req) {
    try {
      ...
      const GumastaStateConfigFound =
        await this.GumastaStateConfigModel.findOne({
          _id: new mongoose.Types.ObjectId(id),
          isDeleted: false,
        });

      if (!GumastaStateConfigFound) {
        throw new HttpException("State not found.", HttpStatus.OK);
      }
      ...
      const result = await this.GumastaStateConfigModel.findByIdAndUpdate(
        { _id: GumastaStateConfigFound._id },
        {
          $set: { ...req.body },
        },
        { new: true }
      ...
      if (!result) {
        throw new HttpException("Could not update state.", HttpStatus.OK);
      }

      /** update district in district module***/

      const stateExistsInDistrictModule =
        await this.GumastaDistrictConfigModel.find({
          state: GumastaStateConfigFound.state.toUpperCase(),
          isDeleted: false,
        });

      if (stateExistsInDistrictModule) {
        for (let each in stateExistsInDistrictModule) {
          let { _id } = stateExistsInDistrictModule[each];
          const result =
            await this.GumastaDistrictConfigModel.findByIdAndUpdate(
              { _id: _id },
              {
                $set: { state: req.body.state.toUpperCase() },
              },
              { new: true }
            );
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        "GUMASTA_STATE_CONFIG",
        "GUMASTA_STATE_CONFIG_UPDATE",
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ""
        } updated gumasta states  ${id} at ${currentDate}.`,
        "Gumasta states updated successfully.",
        req.socket.remoteAddress
      );
      return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ""
        } tried to update  states ${id} with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
        errData.resData.message,
        req.socket.remoteAddress
  ...
  async delete_by_id(id: string, res, req) {
    try {
      ...
      const dataUpdated = await this.GumastaStateConfigModel.findByIdAndUpdate(
        { _id: new mongoose.Types.ObjectId(id) },
        { $set: { isDeleted: true } },
        { new: true }
      );
      ...
      /** delete states in  district***/

      const stateExistsInDistrictModule =
        await this.GumastaDistrictConfigModel.find({
          state: GumastaStateConfigFound.state,
          isDeleted: false,
        });
      if (stateExistsInDistrictModule) {
        for (let each in stateExistsInDistrictModule) {
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        "GUMASTA_STATE_CONFIG",
        "GUMASTA_STATE_CONFIG_DELETE",
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ""
        } deleted gumasta states ${id} at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ""} ${
          req.userData?.contactNumber || ""
        } tried to delete state ${id} with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
        errData.resData.message,
        req.socket.remoteAddress
  ...
  //-------------------------------------------------------------------------
  /***
   * change status of GumastaStateConfig
   */
  //-------------------------------------------------------------------------
  ...
  async changeActiveStatus(id: string, res, req) {
    try {
      ...
      const activeStatus =
        GumastaStateConfigFound.isActive === true ? false : true;
      const statusValue = activeStatus === true ? "ACTIVE" : "DEACTIVE";

      const statusChanged =
        await this.GumastaStateConfigModel.findByIdAndUpdate(
      ...
      if (statusChanged.isActive === activeStatus) {
        this.addLogFunction.logAdd(
          ...
          requestedId,
          "GUMASTA_STATE_CONFIG",
          "GUMASTA_STATE_CHANGE_STATUS",
          id,
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ""
          } status changed of states  ${id} at ${currentDate}.`,
          `Status changed of states ${id} to ${statusValue} successfully.`,
          req.socket.remoteAddress
        );
        return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ""} ${
          req.userData?.contactNumber || ""
        } tried to change status of states ${id} with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
        errData.resData.message,
        req.socket.remoteAddress
  ...
  async allWithFilters(req, res) {
    try {
      ...
      //-----------------------------------
      let totalData = 0;
      const dataFound = await this.GumastaStateConfigModel.aggregate(
        countQuery
      );
      ...
      const result = await this.GumastaStateConfigModel.aggregate(query);
      if (result.length) {
        this.addLogFunction.logAdd(
          req,
          requestedType,
          requestedId,
          "GUMASTA_STATE_CONFIG",
          "GUMASTA_STATE_FILTER_PAGINATION",
          "",
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ""
          }filter data to view states at ${currentDate}.`,
          "Data Found successfully.",
          req.socket.remoteAddress
        );
        return res.status(200).send({
