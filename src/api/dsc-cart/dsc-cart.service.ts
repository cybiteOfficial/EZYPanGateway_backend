...
import { DscCart, DscCartDocument } from "./entities/dsc-cart.entity";
...
@Injectable()
export class DscCartService {
  [x: string]: any;
  constructor(
    @InjectModel(DscCart.name)
    private DscCartModel: Model<DscCartDocument>,
    @InjectModel(DigitalSignFlow.name)
    private DigitalSignFlowModel: Model<DigitalSignFlowDocument>,
    private readonly addLogFunction: AddLogFunction,
    private readonly DscCartHelper: DscCartHelper,
    ...
    try {
      ...
      if (
        ...
        throw new HttpException(
          `You are not allow to access this DSC service.`,
          HttpStatus.OK
        );
      }
      ...
      const DscCartHelper = await this.DscCartHelper.dscCartItemValidation(
        req.body.dscCartItems,
        req,
        res
      );
      //get total price
      const checkAmountForUser = await this.DscCartHelper.getAmount(
        req.userData.type
      );

      DscCartHelper.forEach((el) => {
        el.srn = generateSRN(serviceType.DSC);
        ...
        el.status = status.BLANK; //status enum not working here
      ...
      const cartData = await this.DscCartModel.insertMany(DscCartHelper);
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ""} ${req.userData?.contactNumber || ""} ${
          req.userData?.mobileNumber || ""
        } tried to add to cart DSC  with this ${req.userData.mobileNumber}.`,
  ...
  async view(id: string, res, req) {
    try {
      ...
      const DscItemsFound = await this.DscCartModel.find(query);

      if (!DscItemsFound.length) {
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ""
        }viewed DSC cart  ${id} at ${currentDate}.`,
  ...
  async delete_by_id(id: string, res, req) {
    try {
      const currentDate = moment()
        .utcOffset("+05:30")
        .format("YYYY-MM-DD HH:mm:ss");
      const cartFound = await this.DscCartModel.findOne({
      ...
      const dataUpdated = await this.DscCartModel.findByIdAndUpdate(
        { _id: new mongoose.Types.ObjectId(id) },
        { $set: { isDeleted: true } },
        { new: true }
      );
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData.type}  ${
          req.userData.Id
        }  tried to delete dsc cart with this credentials at ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")}.`,
        errData.resData.message,
        req.socket.remoteAddress
  ...
  async checkout(req, res) {
    try {
      ...
      const cartItems = await this.DscCartModel.find({
        appliedById: req.userData.Id,
        isDeleted: false,
      });
      ...
      if (isRefundApplied) {
        let isRefundExist = await this.DscCartHelper.getRefundAmount(
          requestedId
        );
      ...
      if (isRewardApplied) {
        let isRewardExist = await this.DscCartHelper.getRewardAmount(
          requestedId
        );
        let remainingAmount = totalAmountForGateway;
      ...
      const transactionDataToBeAdded = {
        ...
        remark: `Transaction created for dsc application on ${moment()
          .utcOffset("+05:30")
          .format("YYYY-MM-DD HH:mm:ss")} `,
        reqData: [],
        resData: [],
      ...
      if (statusOfPayment == paymentStatus.SUCCESS) {
        ...
        cartItems.forEach(async (el) => {
          ...
          // let srnToUpdate = generateSRN(serviceType.DSC);
        ...
        cartData = await this.DigitalSignModel.insertMany(cartItems);

        // delete applications from cart
        for (const item in cartData) {
          let { _id } = cartData[item];
          let deleteCartItems = await this.DscCartModel.findByIdAndDelete(_id);
        }
      }
      if (statusOfPayment !== paymentStatus.SUCCESS) {
        ...
        for (const item of cartData) {
          const dscAdd = await extractFieldsForDSCFlowTable(item);
          const updatedPanFlow = await new this.DigitalSignFlowModel({
            digitalSignId: item._id,
            ...dscAdd,
          }).save();
        }
        ...
        for (const item in cartData) {
          ...
          let deleteCartItems = await this.DscCartModel.findByIdAndDelete(
      ...
      if (
        ...
        // cartData[0].appliedByType !== Role.GUEST &&
        isRewardToBeUpdate === true
      ) {
