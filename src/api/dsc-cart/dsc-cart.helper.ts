...
export class DscCartHelper {
  ...
  async dscCartItemValidation(dscCartItems, req, res) {
    try {
      const dataToBeInsert = [];
      for (const eachItem in dscCartItems) {
        let {
          propritorName,
          srn,
          email,
        ...
        // const isValid = await keyValidationCheck(item, reqParams, requiredKeys);
        // if (!isValid.status) {
        ...
        dataToBeInsert.push(dscCartItems[eachItem]);
      }
