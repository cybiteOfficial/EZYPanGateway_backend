...
export class DownloadFileService {
  [x: string]: any;
  constructor(
    @InjectModel(DownloadFile.name)
    private DownloadFileModel: Model<DownloadFileDocument>,
  ...
  async add(res, req) {
    try {
      const { fileTitle, fileUrl, description } = req.body;

      const reqParams = ['fileTitle', 'fileUrl', 'description'];
      const requiredKeys = ['fileTitle', 'fileUrl'];
      ...
      /**check fileurl is valid or not */
      if (isvalidUrl(fileUrl) === false) {
        throw new HttpException('File url must be valid url.', HttpStatus.OK);
      }

      const result = await new this.DownloadFileModel({ ...req.body }).save();
      ...
      this.addLogFunction.logAdd(
        ...
        } added download file at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to add download file with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
  ...
  async list(res, req) {
    try {
      ...
      const dataFound = await this.DownloadFileModel.find({
        ...query,
      });
      if (!dataFound.length) {
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } checked list of download file at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to check list of download file with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async view(id: string, res, req) {
    try {
      ...
      const DownloadFileFound = await this.DownloadFileModel.findOne({
        ...query,
      });
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } viewed download file  ${id} at ${currentDate}.`,
        'Data found successfully.',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to view download file ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async update_by_id(id: string, res, req) {
    try {
      ...
      /**check fileurl is valid or not */
      if (req.body.fileUrl && isvalidUrl(req.body.fileUrl) === false) {
        throw new HttpException('File url must be valid url.', HttpStatus.OK);
      }
      ...
      const result = await this.DownloadFileModel.findByIdAndUpdate(
        { _id: DownloadFileFound._id },
        {
          $set: { ...req.body },
        },
        { new: true },
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } updated download file ${id} at ${currentDate}.`,
  ...
  async delete_by_id(id: string, res, req) {
    try {
      ...
      const dataUpdated = await this.DownloadFileModel.findByIdAndUpdate(
        { _id: new mongoose.Types.ObjectId(id) },
        { $set: { isDeleted: true } },
        { new: true },
      );
      ...
      if (!dataUpdated) {
        ...
        if (fs.existsSync(dataUpdated.fileUrl)) {
          fs.unlinkSync(dataUpdated.fileUrl);
        }
      }
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } deleted download file ${id} at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to delete download file ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async changeActiveStatus(id: string, res, req) {
    try {
      ...
      const activeStatus = DownloadFileFound.isActive === true ? false : true;
      const statusValue = activeStatus === true ? 'ACTIVE' : 'DEACTIVE';

      const statusChanged = await this.DownloadFileModel.findByIdAndUpdate(
      ...
      if (statusChanged.isActive === activeStatus) {
        this.addLogFunction.logAdd(
          ...
          requestedId,
          'DOWNLOAD_FILE',
          'DOWNLOAD_FILE_CHANGE_STATUS',
          id,
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ''
          } status changed of download file ${id} at ${currentDate}.`,
          `Status changed of file ${id} to ${statusValue} successfully.`,
          req.socket.remoteAddress,
        );
        return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${req.userData?.type || ''} ${
          req.userData?.contactNumber || ''
        } tried to change status of download file ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async allWithFilters(req, res) {
    try {
      ...
      //-----------------------------------
      let totalData = 0;
      const dataFound = await this.DownloadFileModel.aggregate(countQuery);
      ...
      const result = await this.DownloadFileModel.aggregate(query);
      if (result.length) {
        this.addLogFunction.logAdd(
          req,
          requestedType,
          requestedId,
          'DOWNLOAD_FILE',
          'DOWNLOAD_FILE_FILTER_PAGINATION',
          '',
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ''
          }filter data to view download file at ${currentDate}.`,
          'Data Found successfully.',
          req.socket.remoteAddress,
        );
        return res.status(200).send({
