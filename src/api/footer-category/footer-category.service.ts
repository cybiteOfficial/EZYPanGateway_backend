...
@Injectable()
export class FooterCategoryService {
  constructor(
  ...
  async add(CreateFooterCategoryDto: CreateFooterCategoryDto, res, req) {
    try {
      ...
      const titleExists = await this.FooterCategoryModel.findOne({
        title: CreateFooterCategoryDto.title,
        isDeleted: false,
      });
      if (titleExists) {
        throw new HttpException('FooterCategory already exist.', HttpStatus.OK);
      }

      const result = await new this.FooterCategoryModel(
      ...
      this.addLogFunction.logAdd(
        ...
        } added footer catgeory at ${currentDate}.`,
        'Footer category added successfully',
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } tried to add footer category with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
  ...
  async list(res, req) {
    try {
      ...
      const currentDate = moment()
        .utcOffset('+05:30')
        .format('YYYY-MM-DD HH:mm:ss');
      const dataFound = await this.FooterCategoryModel.find();
      if (!dataFound.length) {
      ...
      this.addLogFunction.logAdd(
        ...
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } check list of footer category at ${currentDate}.`,
        'Footer category found successfully',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
        message: 'Footer category found successfully.',
        status: true,
        data: dataFound,
        code: 'OK',
        issue: null,
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } tried to check list of footer category with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async view(id: string, res, req) {
    try {
      ...
      const currentDate = moment()
        .utcOffset('+05:30')
        .format('YYYY-MM-DD HH:mm:ss');
      const FooterCategoryFound = await this.FooterCategoryModel.findOne({
        _id: new mongoose.Types.ObjectId(id),
        isDeleted: false,
      });
      if (!FooterCategoryFound) {
        throw new HttpException('Data not found.', HttpStatus.OK);
      }
      this.addLogFunction.logAdd(
        req,
        ...
        requestedId,
        'FOOTER_CATEGORY',
        'FOOTER_CATEGORY_VIEW',
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } viewed footer category ${id} at ${currentDate}.`,
        'Footer category found successfully',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
        message: 'Footer category successfully.',
        status: true,
        data: FooterCategoryFound,
        code: 'OK',
        issue: null,
      });
    } catch (err) {
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } tried to view footer category ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async update_by_id(
    id: string,
    updateFooterCategoryDto: UpdateFooterCategoryDto,
    res,
    req,
  ) {
    const currentDate = moment()
    ...
    try {
      ...
      const titleExists = await this.FooterCategoryModel.findOne({
        title: updateFooterCategoryDto.title,
        isDeleted: false,
        _id: { $ne: new mongoose.Types.ObjectId(id) },
      });
      if (titleExists) {
        throw new HttpException('FooterCategory alreay exist.', HttpStatus.OK);
      }

      const result = await this.FooterCategoryModel.findByIdAndUpdate(
        { _id: FooterCategoryFound._id },
        {
          $set: { ...req.body },
        },
        { new: true },
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        'FOOTER_CATEGORY',
        'FOOTER_CATEGORY_UPDATE',
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } updated footer category ${id} at ${currentDate}.`,
        'Footer category updated successfully',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } tried to update footer category ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async delete_by_id(id: string, res, req) {
    try {
      ...
      const dataUpdated = await this.FooterCategoryModel.findByIdAndUpdate(
        { _id: new mongoose.Types.ObjectId(id) },
        { $set: { isDeleted: true } },
        { new: true },
      );
      ...
      this.addLogFunction.logAdd(
        ...
        requestedId,
        'FOOTER_CATEGORY',
        'FOOTER_CATEGORY_DELETE',
        id,
        true,
        200,
        `${requestedType} ${
          req.userData?.contactNumber || ''
        } deleted footer category ${id} at ${currentDate}.`,
        'Footer category deleted successfully',
        req.socket.remoteAddress,
      );
      return res.status(200).send({
  ...
  async changeActiveStatus(id: string, res, req) {
    try {
      ...
      const activeStatus = FooterCategoryFound.isActive === true ? false : true;
      const statusValue = activeStatus === true ? 'ACTIVE' : 'DEACTIVE';

      const statusChanged = await this.FooterCategoryModel.findByIdAndUpdate(
      ...
      if (statusChanged.isActive === activeStatus) {
        this.addLogFunction.logAdd(
          ...
          requestedId,
          'FOOTER_CATEGORY',
          'FOOTER_CATEGORY_STATUS',
          id,
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ''
          } changed status footer category ${id} at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } tried to change status of footer category ${id} with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
  ...
  async allWithFilters(req, res) {
    try {
      ...
      //-----------------------------------
      let totalData = 0;
      const dataFound = await this.FooterCategoryModel.aggregate(countQuery);
      ...
      const result = await this.FooterCategoryModel.aggregate(query);
      if (result.length) {
        this.addLogFunction.logAdd(
          req,
          requestedType,
          requestedId,
          'FOOTER_CATEGORY',
          'FOOTER_CATEGORY_FILTER_PAGINATION',
          '',
          true,
          200,
          `${requestedType} ${
            req.userData?.contactNumber || ''
          } filter data to view footer category at ${currentDate}.`,
          'Footer category found successfully',
          req.socket.remoteAddress,
        );
        return res.status(200).send({
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } tried to filter data to view footer category with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
