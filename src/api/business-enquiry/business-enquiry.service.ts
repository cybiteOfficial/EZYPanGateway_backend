...
export class BusinessEnquiryService {
  [x: string]: any;
  constructor(
    @InjectModel(BusinessEnquiry.name)
    private BusinessEnquiryModel: Model<BusinessEnquiryDocument>,
  ...
  async add(res, req) {
    try {
      ...
      const result = await new this.BusinessEnquiryModel({
        ...req.body,
      }).save();
      ...
      this.addLogFunction.logAdd(
        ...
        req.userData?.Id || '',
        'BUSINESS_ENQUIRY',
        'BUSINESS_ENQUIRY_ADD',
        result._id,
        true,
        200,
        `${req.userData?.type || ''} added business enquiry at ${currentDate}.`,
        'Business enquiry added successfully.',
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        }  tried to add business enquiries with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
  ...
  async allWithFilters(req, res) {
    try {
      ...
      //-----------------------------------
      let totalData = 0;
      const dataFound = await this.BusinessEnquiryModel.aggregate(countQuery);
      ...
      const result = await this.BusinessEnquiryModel.aggregate(query);
      if (result.length) {
        this.addLogFunction.logAdd(
          req,
          req.userData?.type || '',
          req.userData.Id,
          'BUSINESS_ENQUIRY',
          'BUSINESS_ENQUIRY_FILTER_PAGINATION',
          req.userData?.Id || '',
          true,
          200,
          `${req.userData?.type || ''} ${
            req.userData?.Id || ''
          } filter data for business enquiry  at ${currentDate}.`,
      ...
      this.addLogFunction.logAdd(
        ...
        `${
          req.userData?.type || ''
        } filter data for view buisiness enquiry with this credentials at ${moment()
          .utcOffset('+05:30')
          .format('YYYY-MM-DD HH:mm:ss')}.`,
        errData.resData.message,
        req.socket.remoteAddress,
