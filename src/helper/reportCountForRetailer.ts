...
export class reportForRetailer {
  [x: string]: any;
  constructor(
  ...
  async getAdmin(retailerQuery, applicationQuery) {
    const retailers = await this.userModel.aggregate([
      {
        $match: { ...retailerQuery },
      },
    ]);
    ...
    for (const each in retailers) {
      ...
      const retailer = retailers[each];
      const retailerId = retailer._id.toString();
      const retailerName = retailer.name;
      const retailerMobileNumber = retailer.mobileNumber;

      const orQuery = {
        isDeleted: false,
        appliedById: retailerId,
      };
      ...
      const getPanCounts = await this.panModel.aggregate([
        ...
        },
        {
          $project: {
            ...
            retailerMobileNumber: retailerMobileNumber,
          },
        },
        {
          $group: {
  ...
  async formattedData(createData: any[]) {
    return createData?.map((data) => {
      return {
        ...
        retailerMobileNumber: data?.retailerMobileNumber,
        pan: {
